...
/*   Updated: 2022/08/03 19:07:12 by rkultaev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// long long	ms;
	...
	while (!die)
	{
		...
		{
			print_message(data, philo->idx, "iss died");
			// ms = curr_time_millisec() - data->start_time;
...
void	*check_amount_of_meal(void *philo_check)
{
	...
	while (1)
	{
		pthread_mutex_lock(&data->stop_mutex);
		// pthread_mutex_lock(&data->stop_mutex);
		if ((data->nb_meals_stop_philo == data->nb_of_philos) || data->stop == 1)
		{
			data->stop = 1;
			pthread_mutex_unlock(&data->stop_mutex);
			break ;
